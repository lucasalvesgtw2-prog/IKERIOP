...
| 4   | Role assignment           | creator                                     | One select: _Who is the Buyer?_ The seller is derived as the other participant, so an invalid combination cannot be expressed. The server re-checks it anyway, and the database enforces one BUYER and one SELLER row per deal. A **Swap Buyer / Seller** button stays available until the seller submits details. |
